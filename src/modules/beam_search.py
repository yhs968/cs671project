...
def generate_title(doc_sents, beam_size, max_kernel_size, models, max_target_length = 100, batch_size = 1, verbose = False):
    '''
    Args:
    ...
    # Ignore if the content is a single sentence
    if len(sents_raw) <= 1:
        raise Exception('Error: The length of the document is %i.' % len(sents_raw))   
...
#     print(sents_ext)
    ...
    if len(sents_ext) == 0:
        raise Exception('No sentences are selected')
...
#     print(words)
    ...
    if len(abs_enc_output) > 6000:
        raise Exception('The document is too long')
